...
module axial_ring(outer_diameter, thickness, spacing, height, resolution, innermost = false, outermost = false)
{
    ...
    axis_height = min(thickness*2 - epsilon, height*0.5);
    ...
    difference() {
        ...
        union() {
            ...
            if(!innermost) {
                ...
                translate([0, (inner_radius + epsilon), height / 2]) {
                    rotate([90,0,0]) {
                        cylinder(r1 = axis_height, r2 = 0, h = axis_height);
                    }
                }
                //cone 2
                translate([0, -(inner_radius + epsilon), height / 2]) {
        ...
        if(!outermost) {
            ...
            translate([-outer_radius, 0, height / 2]) {
                rotate([0,90,0]) {
                    cylinder(r1 = axis_height, r2 = 0, h = axis_height - spacing + axis_spacing);
                }
            }
            //cone 2
            translate([outer_radius, 0, height / 2]) {
